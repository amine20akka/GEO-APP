...
export class QuickChatComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 /**
     * Fetch layers from workspace
     */
 fetchLayers(): void {
    const workspace = 'test_data'; // Replace with your actual workspace name
    this._layersService.fetchLayersFromWorkspace(workspace)
        .then(data => {
...
dragIndex: number;
  ...
  onDragStart(event: DragEvent, index: number) {
    event.dataTransfer.setData('text', ''); // Firefox requires setData to be called
  ...
  onDrop(event: DragEvent) {
    event.preventDefault();
    const toIndex = this.findIndex(event);
    if (toIndex !== -1 && this.dragIndex !== toIndex) {
      const removedItem = this.layers.splice(this.dragIndex, 1)[0];
      this.layers.splice(toIndex, 0, removedItem);
    }
  }

  private findIndex(event: DragEvent): number {
    const target = event.target as HTMLElement;
    const parent = target.closest('.flex-auto') as HTMLElement;
    const children = parent.getElementsByClassName('flex');
...
selectLayer(layer: Layer): void {
    ...
    if(!this.opened){
        console.log(layer);
        if (!this.clicked){ this._mapService.addWMSLayer(layer.name);this.clicked = true}
        else{ this._mapService.removeWMSLayer(layer.name);this.clicked = false}
